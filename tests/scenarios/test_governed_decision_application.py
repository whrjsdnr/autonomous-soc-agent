...
The real in-memory CAS implementation performs all applications and replay checks.
"""
...
async def test_packaged_models_through_governed_application(packages, case, monkeypatch):
    state = IncidentState()
    evidence = Evidence(
    ...
    store = InMemoryIncidentStateStore((state, other_state))
    ...
    if case in ("review_only", "investigate"):
        ...
        if case == "stale":
            ...
            assert store.load(state.incident_id).state == state
            assert store.applications() == ()
            assert service.failures()[-1].error_type == "RuntimeError"
        else:
